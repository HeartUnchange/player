...
Player::Player(const std::string &file_name) : demuxer_{std::make_unique<Demuxer>(file_name)},
											   ...
											   video_frame_queue_{std::make_unique<FrameQueue>(video_queue_size_)},
											   audio_frame_queue_{std::make_unique<FrameQueue>(audio_queue_size_)}
{
}
...
void Player::cleanup()
{
	audio_frame_queue_->quit();
	video_frame_queue_->quit();
		...
		for (;;)
		{
			...
			while (!sent)
			{
				...
				while (video_decoder_->receive(frame_decoded.get()))
				{
					...
					if (!video_frame_queue_->push(move(frame_converted)))
					{
						break;
					}
				}
